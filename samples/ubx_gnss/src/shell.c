				 ...
				 int64_t time_utc)
{
	...
	LOG_INF("Found position estimate: (lat, lon): (%f, %f), alt: "
		"%.2fm, radius: %.2fm (%d SV used)", latitude, longitude,
		altitude_meters, radius_meters, space_vehicles_used);
#else
	printk("Found position estimate: (lat, lon): (%f, %f), alt: "
	       "%.2fm, radius: %.2fm (%d SV used)\n", latitude, longitude,
	       altitude_meters, radius_meters, space_vehicles_used);
#endif
...
static int cmd_gnss_single(const struct shell *sh, size_t argc, char **argv, void *data)
{
	...
	int ret = uGnssPosGet(gnss_device_handle, &latitudeX1e7,
						  &longitudeX1e7,
						  &altitude_millimeters,
						  &radius_millimeters,
	...
	shell_print(sh, "Found position estimate after %.1fs: "
			"(lat, lon): (%f, %f), alt: %.2fm, radius: "
			"%.2fm (%d SV used)", time_to_fix_seconds,
			latitude, longitude, altitude_meters,
			radius_meters, space_vehicles_used);
...
static int cmd_gnss_ttff(const struct shell *sh, size_t argc, char **argv, void *data)
{
	...
	for (int i = 0; i < n_tests; i++) {
		...
		uint64_t start_time_ms, stop_time_ms, duration_ms;
		int ret;

		reset_gnss();

		start_time_ms = k_uptime_get();
		ret = uGnssPosGet(gnss_device_handle, NULL, NULL, NULL, NULL,
						      NULL, NULL, NULL, NULL);
		if (ret != 0) {
			shell_error(sh, "Error during uGnssPosGet: %d", ret);
			return -1;
		...
		stop_time_ms = k_uptime_get();
		duration_ms = stop_time_ms - start_time_ms;

		shell_print(sh, "Run %d of %d: Acquired fix after %.2fs",
				i+1, n_tests, duration_ms / 1000.0);
