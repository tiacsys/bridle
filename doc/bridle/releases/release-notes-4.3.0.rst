...
* Enable support for the SiFli HAL in the West manifest. This HAL is needed by
  Zephyr upstream documentation process for the autogenerated board catalog.
* Using :command:`west` package manager dependencies with :command:`uv` on all
  GitHub CI workflows, especially QA integration and documentation build.
