...
/*
 * Asynchronous message transfers are processed by a tracking structure being
 * passed between 3 separate work items: One for initiating transfers, one for
 * performing RX buffer readouts, and one for awaiting completion of one message
 * being processed.
 *
 ...
 * processing.
 *
 * On completion, or if unrecoverable errors occur, each work item can call out
 * to a shared completion function, which propagates the final return value to
 * the user provided callback.
 *
 * A driver instance wide lock ensures that only one packet of messages given to
 * `i2c_transfer_cb` is processed at the same time. Multiple calls can still be
 * made in succession, but it is not guaranteed that they will be processed in
...
size_t size = sizeof(transfer_work_t);

/* Finish an asynchronous transfer with some result, cleaning up locks and resources. */
static void i2c_sc18is604_transfer_cb_process_done(transfer_work_t *transfer_work,
						   int result)
{
...
{
	transfer_work_t *transfer_work = CONTAINER_OF(work, transfer_work_t, work_init_msg);
	const struct device *dev = transfer_work->dev;
	i2c_sc18is604_data_t * const data = dev->data;
	int ret = 0;
...
{
	transfer_work_t *transfer_work = CONTAINER_OF(work, transfer_work_t, work_buffer_readout);
	const struct device *dev = transfer_work->dev;
	const i2c_sc18is604_config_t * const config = dev->config;
	i2c_sc18is604_data_t * const data = dev->data;
	int ret = 0;
	...
	if (!signaled) {
		/*
		 * We didn't reset the transfer signal, so we'll fall through to
		 * awaiting the interrupt again next time.
		 */
		ret = k_work_submit(work);
		if (ret != 1 && ret != 0) {
			i2c_sc18is604_transfer_cb_process_done(transfer_work, -EIO);
	...
	/*
	 * Check contents of I2C_STATUS register (propagated through interrupt
	 * signal)
	 */
...
{
	transfer_work_t *transfer_work = CONTAINER_OF(work, transfer_work_t, work_finish_msg);
	const struct device *dev = transfer_work->dev;
	i2c_sc18is604_data_t * const data = dev->data;
	int ret = 0;
