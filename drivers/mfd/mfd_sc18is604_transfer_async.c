...
static void mfd_sc18is604_transfer_work_fn(struct k_work *work)
{
	...
	ret = mfd_sc18is604_transfer(dev,
				     transfer_work->cmd,
				  ...
				  struct k_poll_signal *signal)
{
	...
	*transfer_work = (struct mfd_sc18is604_transfer_work) {
	...
	/*
	 * We can expect the TX and RX buffer to stay live until transfer
	 * completion, but the command sequence was stack-allocated inbetween. To
	 * make sure it stays valid, we create an allocated copy attached to the
	 * transfer work.
	 */
