...
static void mfd_sc16is75x_interrupt_work_fn_init(struct k_work *work)
{
	...
	/*
	 * Attempt to get interrupt handling lock. If it's taken, that means a
	 * handling transaction is already active. In that case, reschedule
	 * self.
	 */
	...
	for (int i = 0; i < config->n_channels; i++) {
		...
		if (ret != 0) {
			 ...
			 * a failure to complete it: Have the finalizing work
...
static void mfd_sc16is75x_interrupt_work_fn_final(struct k_work *work)
{
	...
	struct mfd_sc16is75x_data * const data = CONTAINER_OF(work_delayable,
			struct mfd_sc16is75x_data, interrupt_work_final);
	const struct device * const dev = data->self;
	const struct mfd_sc16is75x_config * const config = dev->config;
	...
	/*
	 * Resubmit handler to queue to look for further pending interrupts, or
	 * if interrupt pin is still active.
	 */
