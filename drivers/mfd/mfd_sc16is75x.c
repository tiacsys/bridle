...
static void mfd_sc16is75x_interrupt_work_fn_final(struct k_work *work)
{
	...
	for (int i = 0; i < config->n_channels; i++) {
		...
		k_poll_signal_check(&data->interrupt_data.signals[i],
				    &signaled, &results[i]);
		if (!signaled) {
			/*
	...
	for (int i = 0; i < config->n_channels; i++) {
		uint8_t type = GET_FIELD(data->interrupt_data.iir[i],
					 SC16IS75X_BIT_IIR_TYPE);
		...
		if (IS_BIT_SET(data->interrupt_data.iir[i],
...
static int mfd_sc16is75x_init(const struct device *dev)
{
	...
	k_work_init(&data->interrupt_work_init,
		    mfd_sc16is75x_interrupt_work_fn_init);
