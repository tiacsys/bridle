...
#define REG_OUTPUT_DFLT		0xFF
#define REG_POL_INV_PORT	0x02
#define REG_POL_INV_DFLT	0x00
			  ...
			  uint8_t *cache, uint8_t *buf)
{
	...
	ret = i2c_burst_read_dt(&config->bus, reg, (uint8_t *)&port_data,
				sizeof(port_data));
	if (ret != 0) {
		LOG_ERR("PCA9554[0x%X]: error reading register 0x%X (%d)",
...
// FIXME: __REMOVE__ 	value = sys_le16_to_cpu(port_data);
	...
	*buf = value;
	...
	LOG_DBG("PCA9554[0x%X]: Read: REG[0x%X] = 0x%X, REG[0x%X] = 0x%X",
			   ...
			   uint8_t *cache, uint8_t value)
{
	...
	LOG_DBG("PCA9554[0x%X]: Write: REG[0x%X] = 0x%X, REG[0x%X] = "
...
// FIXME: __REMOVE__ 	sys_put_le16(value, &buf[1]);
	...
	if (ret == 0) {
		*cache = value;
		...
		LOG_ERR("PCA9554[0x%X]: error writing to register 0x%X "
			       ...
			       gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (ret) {
		LOG_ERR("PCA9554[0x%X]: error setting pin direction (%d)",
			config->bus.addr, ret);
		goto done;
	}
						  ...
						  enum gpio_int_trig trig)
{
	...
	if ((mode != GPIO_INT_MODE_DISABLED) &&
		...
		LOG_ERR("PCA9554[0x%X]: output pin cannot trigger interrupt",
			config->bus.addr);
		return -ENOTSUP;
	}
	...
	if (active != drv_data->interrupt_active) {
		...
		if (ret != 0) {
			LOG_ERR("PCA9554[0x%X]: failed to configure interrupt "
...
/**
 * @brief Reset defaults function of PCA9554
 *
...
static int gpio_pca9554_reset_defaults(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERR("PCA9554[0x%X]: error reset port direction (%d)",
			config->bus.addr, ret);
		goto err;
	}
	...
	if (ret != 0) {
		LOG_ERR("PCA9554[0x%X]: error reset port output (%d)",
			config->bus.addr, ret);
		goto err;
	}
...
static int gpio_pca9554_init(const struct device *dev)
{
	...
	if ((config->capabilities & PCA_HAS_INTERRUPT) != 0) {
		...
		if (!device_is_ready(config->int_gpio.port)) {
			LOG_ERR("PCA9554[0x%X]: interrupt GPIO not ready",
				config->bus.addr);
			return -ENODEV;
		}
