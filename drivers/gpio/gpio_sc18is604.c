 ...
 */
typedef struct gpio_sc18is604_config {
 ...
 */
typedef struct gpio_sc18is604_data {
	...
	uint8_t pin_state;
} gpio_sc18is604_data_t;

static int gpio_sc18is604_pin_write_config(const struct device *dev, uint8_t val)
{
	const gpio_sc18is604_config_t * const config = dev->config;
	gpio_sc18is604_data_t * const data = dev->data;
	int ret = 0;

	/* FIXME: provide macro */
	ret = mfd_sc18is604_write_register(config->bridge, SC18IS604_REG_IO_CONFIG, val);
	if (ret != 0) {
		goto end;
	}
...
static int gpio_sc18is604_pin_read_config(const struct device *dev, uint8_t *val)
{
	...
	/* FIXME: provide macro */
	ret = mfd_sc18is604_read_register(config->bridge, SC18IS604_REG_IO_CONFIG, val);
	if (ret != 0) {
		goto end;
	}
...
static int gpio_sc18is604_pin_write_state(const struct device *dev, uint8_t val)
{
	...
	/* FIXME: provide macro */
	ret = mfd_sc18is604_write_register(config->bridge, SC18IS604_REG_IO_STATE, val);
	if (ret != 0) {
		goto end;
	}
...
static int gpio_sc18is604_pin_read_state(const struct device *dev, uint8_t *val)
{
	...
	/* FIXME: provide macro */
	ret = mfd_sc18is604_read_register(config->bridge, SC18IS604_REG_IO_STATE, val);
	if (ret != 0) {
		goto end;
	}
					 ...
					 uint32_t mask, uint32_t value)
{
	...
	pin_state = data->pin_state;
	pin_state = (pin_state & ~mask) | (mask & value); /* FIXME: BYTE macro */
...
static int gpio_sc18is604_toggle_bits(const struct device *dev, uint32_t pins)
{
	...
	pin_state = data->pin_state;
	pin_state ^= pins; /* FIXME: BYTE macro */
...
static int gpio_sc18is604_init(const struct device *dev)
{
	...
	LOG_DBG("%s: ready for %u pins with bridge over %s!", dev->name,
		config->num_pins, config->bridge->name);
	return 0;
}
...
#define GPIO_SC18IS604_DEFINE(inst)                                          \
                                                                             \
	static const gpio_sc18is604_config_t gpio_sc18is604_config_##inst =  \
	{                                                                    \
		.common = {                                                  \
			.port_pin_mask =                                     \
	...
	BUILD_ASSERT(                                                        \
		...
		"Too many ngpios");                                          \
                                                                             \
	static gpio_sc18is604_data_t gpio_sc18is604_data_##inst;             \
                                                                             \
