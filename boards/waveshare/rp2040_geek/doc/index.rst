...
* :bridle_file:`boards/waveshare/rp2040_geek/waveshare_rp2040_geek_defconfig`
...
channel PWM4 B is available on the on-board backlight LED. Only the channels
PWM1 A and B are available on one of the three edge connectors. Both PWM
channels will share the same lines with the I2C1 signals.

ADC/TS Ports
============

The `RP2040 <RP2040 SoC_>`_ MCU has 1 ADC with 4 channels and an additional
fifth channel for the on-chip temperature sensor (TS). The ADC channels 2-3
are available on one of the three edge connectors. Both ADC channels will
share the same lines with the I2C0 signals.
...
The `RP2040 <RP2040 SoC_>`_ MCU has 2 SPIs. Neither of the two SPIs is exposed
externally. These are connected internally to the LCD and the TF/microSD card
interfaces.

I2C Port
========

The `RP2040 <RP2040 SoC_>`_ MCU has 2 I2Cs. To one of the three edge connectors
I2C0 is connect to external devices over GP28 (I2C0_SDA) and GP29 (I2C0_SCL).
Teses two I2C0 signals will share the same lines with ADC channels 2 and 3.
I2C0 is enabled by default.

Serial Port
===========

The `RP2040 <RP2040 SoC_>`_ MCU has 2 UARTs. One of the UARTs (UART1) is
connected to external devices over GP4 (TX) and GP5 (RX) on one of the three
edge connectors. UART0 is not enabled by default.
...
.. hint::
   ...
   Neither LED Blinky nor LED Fade can be built and executed on the
   |RP2040-Geek| for an actual user LED, but of course the LCD backlight
   can be used for testing purposes.

.. include:: blinky_fade_lcd_backlight.rsti

.. hint::

   Even with the help of the dedicated :ref:`loopback_test_shield` shield,
   all necessary device tree changes are temporarily carried out to execute
   the standard examples. This assumes the external wiring as shown below
   (on the right).

.. image:: img/loopback_test_shield.jpg
   :align: right
   :alt: Waveshare RP2040-Geek with loopback wiring for tests

.. include:: blinky_fade_test_shield.rsti
